...
void SIMD_render_pointed_celtic(SDL_Renderer* renderer, 
	...
	for (y = 0; y < screen_height; y++)
	{
		...
		for (x = 0; x < screen_width; x += 4)
		{
			...
			for (i = 0; i < f->iter; i++)
			{
				...
				_na = _mm256_andnot_pd(_minus, _mm256_add_pd(_mm256_mul_pd(_a, _mm256_andnot_pd(_minus, _a)), _mm256_mul_pd(_b, _b)));
